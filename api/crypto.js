...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const url = `https://api.twelvedata.com/quote?symbol=${syms}&apikey=${key}`;
    ...
    const results = pairs.map(p => {
      ...
      return { symbol: p.ticker, name: p.name, price, changePercent, high: parseFloat(q.high||0), low: parseFloat(q.low||0) };
    }).filter(c => c.price > 0);
